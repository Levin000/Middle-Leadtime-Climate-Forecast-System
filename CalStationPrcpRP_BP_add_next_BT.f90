  ...
  REAL(KIND = 8), ALLOCATABLE :: ptandPrcp(:), ptor1Prcp(:), ptor2Tavg(:)   !保存预报量站点、第一预报因子、第二预报因子降雨量数据,
...
3000 FORMAT(f13.0,f3.0,f13.0,f3.0,3f6.2)
  ...
  ELSE
    ...
    CALL SLEEP(1)
    CALL Progress % Set( N = ValidStationNum , L = 30 )!// size(ValidPrcpStationCodesIndex)次，显示长度30
  ...
  IF (alive) THEN
    ...
    CLOSE(fileID)
    PRINT *, ">>读取ValidTavgStationCodes.dat文件成功！"
    CALL SLEEP(1)
  END IF
  ! 读取温度站点标准数据库文件
...
96  PRINT *,'Please input StartNum(as integer format ,StartNum should >= 1 and <= 20547):'
  ...
  !初始化命令行进度条
  CALL Progress % Set( N = StudyStationNum001 , L = 30 )!// size(ValidPrcpStationCodesIndex)次，显示长度30
  Progress % M = "=" !// 已完成部分的字符，不是必须
  Progress % O = "." !// 未完成部分的字符，不是必须
  ...
  END DO
  !================================================================================================================
  istatus_dir_ch = CHDIR(TRIM(PrcpWorkSpace)//'add_next_StationList_BT') !设置默认的工作目录
