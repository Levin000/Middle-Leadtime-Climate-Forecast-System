SUBROUTINE PredictLastOneData_BP
      ...
      CHARACTER(LEN = 500) :: cc_Path  !Coupled_capacity_BP.dat路径
      ...
      NAMELIST /PLODBP/ stationMonthNum,GhcnPrcpColNum,StartMonth,MonthNum,RankNum,StartYear,EndYear,ClimateStatus,MissVal,TraceVal,TrainingRate
...
900   FORMAT(i13,i3,i13,i3,3f10.2)
      ...
      cc_Path = TRIM(Path)//'\BaseOnPrcp\CalculateStationPrcpRP\'
...
40      CALL Progress % Set( N = ValidStationNum , L = 30 )!// StationNum次，显示长度30
      ...
      ELSE
        istatus = CHDIR(TRIM(cc_Path))  ! 切换路径到数据库路径
        INQUIRE(FILE = 'Coupled_capacity_BP.dat', EXIST = alive001)
        IF (alive001) THEN
          PRINT *,'>>Coupled_capacity_BP.dat文件存在，读取文件...'
          CALL Inqire_Text_Row('Coupled_capacity_BP.dat',LEN('Coupled_capacity_BP.dat'),tempNum)
          ALLOCATE(ccData(tempNum,13))
          ALLOCATE(RealArrayTemp2D(13,tempNum))
          OPEN(fid,FILE = 'Coupled_capacity_BP.dat')
          ...
          PRINT *,'StudyStationNum: ',StudyStationNum001
          PRINT *,'>>读取Coupled_capacity_BP.dat文件完成!'
        END IF
        tempNum = 0
      ...
      !***********************************************************************************************************
      ! get and save the information of station which can be predicted (全年12个月都满足预报条件的站点)
