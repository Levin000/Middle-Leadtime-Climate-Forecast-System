        ...
        DO j = 1,ValidTavgStationNum
          ...
          DO k = 1,AheadMonthNum
            ...
            DO ii = 1,MonthNum            
		      ...
		      IF ((pstor1ID == pstor2ID) .AND. (K == INT(pstor2LM))) THEN
				CONTINUE
		      ELSE IF (Rtandtor1 < -1) THEN
				CONTINUE
			  ELSE
              ...
              if((isContinuityGT_M(TempMonthStudyPrcpModify,ClimateStatus,prcp_anomaly_missing) .EQ. .false.) .and.&
                ...
                tempCount = COUNT(TempMonthFactorTavgModify>-9998 .AND. TempMonthStudyPrcpModify>=0)
                ALLOCATE(CodesIndexLocation(tempCount))
                tempNum = 0
                DO jj = 1,SIZE(TempMonthFactorTavgModify)
                  IF(TempMonthFactorTavgModify(jj)>-9998 .AND. TempMonthStudyPrcpModify(jj)>=0 .AND. tempFactorTavgMonthModify(jj)>0) THEN
                    tempNum = tempNum + 1
